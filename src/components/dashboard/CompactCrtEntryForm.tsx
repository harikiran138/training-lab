            ...
            <button
              onClick={() => {
                for (const row of data) {
                  const max = row.strength;
                  const days = [row.d1, row.d2, row.d3, row.d4, row.d5, row.d6].map(v => Number(v) || 0);
                  if (days.some(d => d > max)) {
                    alert(`Validation Error: ${row.branch} has attendance greater than total strength (${max}).`);
                    return;
                  }
                }
                onSave(data, locked);
